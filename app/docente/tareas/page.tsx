          ...
          {filtered.map((t, i) => {
            ...
            return (
              <motion.div key={t.id} variants={listItem} initial="hidden" animate="show" exit="exit"
                ...
                className={`bg-sb-surface rounded-2xl overflow-hidden cursor-pointer hover:bg-sb-surface-container-high/50 transition-all duration-200 ${isOverdue ? 'ring-1 ring-red-500/20' : ''}`}>
                <div className="p-5">
                  ...
                  <div className="mt-4">
                    ...
                    <div className="h-2 bg-sb-surface-container rounded-full overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
      ...
      <SbModal open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="520px">
        ...
        <SbModalBody>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            ...
            {/* Fechas */}
            <div className="bg-sb-surface-container/50 rounded-xl p-4 space-y-3">
              ...
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-sb-surface-container/50 rounded-xl p-3">
                    ...
                    <span className={`inline-flex items-center gap-1.5 text-[10px] font-semibold px-2.5 py-1 rounded-full ${statusConfig[selectedTask.status].bg} ${statusConfig[selectedTask.status].color}`}>
                  ...
                  <div className="bg-sb-surface-container/50 rounded-xl p-3">
                    ...
                    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${priorityConfig[selectedTask.priority].bg} ${priorityConfig[selectedTask.priority].color}`}>
                      {priorityConfig[selectedTask.priority].label}
                    </span>
                  </div>
                  <div className="col-span-2 bg-sb-surface-container/50 rounded-xl p-3">
                ...
                <div>
                  ...
                  <div className="bg-sb-surface-container/30 rounded-xl overflow-hidden">
                        ...
                        {selectedTask.students.map((student, i) => {
                          ...
                          return (
                            <motion.div key={student.student_id}
                              ...
                              <div className="flex items-center gap-3">
                                <div className="h-8 w-8 rounded-full bg-sb-on-surface/8 flex items-center justify-center text-[10px] font-bold text-sb-on-surface-variant/60">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className={`inline-flex items-center gap-1.5 text-[10px] font-semibold px-2.5 py-1 rounded-full ${ss.bg} ${ss.color}`}>
                                ...
                                {student.submission_status === 'pending' && (
                                  <button onClick={(e) => { e.stopPropagation(); handleMarkSubmitted(selectedTask.id, student.student_id, student.submission_id) }}
                                    className="text-[10px] font-medium text-blue-600 hover:text-blue-700 bg-blue-500/10 px-2.5 py-1 rounded-full transition-colors">
                                    Marcar entrega
                                  </button>
                                )}
                              </div>
