...
export default function MaterialesPage() {
  ...
  return (
    <div className="w-full space-y-6 py-2">
      ...
      <div className="bg-[var(--sb-surface-container)] rounded-[6px] divide-y divide-[var(--sb-outline-variant)]/15">
        {filtered.map((m, i) => {
          ...
          return (
            <motion.div key={m.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="flex items-center gap-4 px-5 py-4">
              <div className={`h-10 w-10 rounded-[6px] flex items-center justify-center shrink-0 ${fileColors[m.type] || "bg-[var(--sb-surface-container-high)]"}`}><Icon className="h-4 w-4" /></div>
              ...
              <button className="p-2 rounded-[6px] hover:bg-[var(--sb-surface-container-high)] transition-colors shrink-0"><Download className="h-4 w-4 text-[var(--sb-on-surface-variant)]/40" /></button>
            </motion.div>
          )
        })}
