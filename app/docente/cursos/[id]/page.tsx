    ...
    return (
      <div className="w-full h-full rounded-[25px] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] bg-black dark:bg-[#1a1a1c]">
        <div className="p-6 md:p-8 pb-24 md:pb-8 space-y-5">
          <div className="h-6 w-48 bg-[#3f3f46] rounded animate-pulse" />
          <div className="bg-[#17171a] rounded-[30px] p-5 animate-pulse space-y-3">
            <div className="h-11 w-11 rounded-[14px] bg-[#3f3f46]" />
            <div className="h-4 w-40 rounded bg-[#3f3f46]" />
            <div className="h-3 w-24 rounded bg-[#3f3f46]" />
          </div>
          {[1, 2, 3].map(i => (
            <div key={i} className="bg-[#17171a] rounded-[30px] p-5 animate-pulse">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-[14px] bg-[#3f3f46]" />
                <div className="space-y-2">
                  <div className="h-4 w-32 rounded bg-[#3f3f46]" />
                  <div className="h-3 w-20 rounded bg-[#3f3f46]" />
                </div>
              </div>
            </div>
        ))}
  ...
  return (
    <div className="w-full h-full rounded-[25px] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] bg-black dark:bg-[#1a1a1c] sb-note">
        ...
        {quickActions.map((a, i) => {
          ...
          return (
            <motion.div key={a.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 + i * 0.04 }}>
              <Link href={a.href} className="block p-5 rounded-[30px] bg-[#17171a] hover:shadow-md transition-all">
                <div className="h-11 w-11 rounded-[14px] bg-[#27272a] flex items-center justify-center mb-3">
      ...
      <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.05 } } }} className="grid grid-cols-2 gap-3 mt-6">
        <motion.div variants={staggerItem} className="rounded-[30px] bg-[#17171a] p-6">
          <div className="mb-5 h-10 w-10 rounded-[14px] bg-[#27272a] flex items-center justify-center">
          ...
          {students.length === 0 ? (
            ...
            students.map(s => (
              <motion.button
                ...
                className="w-full text-left bg-[#17171a] rounded-[30px] p-4 flex items-center gap-3 hover:-translate-y-px hover:opacity-90 transition-all cursor-pointer group">
                ...
                <span className="inline-flex items-center gap-1.5 text-[10px] font-medium text-[#f4f4f5] bg-[#27272a] px-2.5 py-1 rounded-[12px] opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                  Ver ficha
                </span>
              </motion.button>
            ))
      ...
      {activeTab === "docentes" && (
        <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.04 } } }} className="space-y-2 mt-4">
          {teachers.length === 0 ? (
            ...
            teachers.map(t => (
              <motion.div key={t.id} variants={staggerItem} className="bg-[#17171a] rounded-[30px] p-4 flex items-center gap-3">
      ...
      <div className="mt-6 bg-[#17171a] rounded-[30px] p-4 flex items-start gap-3">
          <div className="h-8 w-8 rounded-[14px] bg-[#27272a] flex items-center justify-center shrink-0">
        ...
        {fichaOpen && (
          <motion.div
            ...
            <motion.div
              ...
              className="relative bg-[#17171a] rounded-[30px] w-full max-w-[560px] shadow-2xl shadow-black/30 max-h-[90vh] overflow-y-auto">
              {fichaLoading ? (
                <div className="p-10 flex flex-col items-center gap-3">
                  <div className="h-8 w-8 border-2 border-[#3f3f46] dark:border-[#3f3f46] border-t-[#000] dark:border-t-[#f4f4f5] rounded-full animate-spin" />
                ...
                <div>
                  <button onClick={() => setFichaOpen(false)}
                    className="absolute right-3 top-3 h-8 w-8 rounded-[14px] flex items-center justify-center bg-[#27272a] text-[#a1a1aa] hover:text-[#f4f4f5] transition-colors z-10">
                    <X className="h-4 w-4" />
                  </button>

                  {/* Header */}
                  <div className="p-6 pb-5 flex items-center gap-4 bg-[#27272a]">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {ficha.student.document_number && (
                          <span className="text-[10px] font-medium px-2 py-0.5 rounded-[12px] bg-[#3f3f46] text-[#a1a1aa]">DNI: {ficha.student.document_number}</span>
                        )}
                        {ficha.student.gender && (
                          <span className="text-[10px] font-medium px-2 py-0.5 rounded-[12px] bg-[#3f3f46] text-[#a1a1aa]">{ficha.student.gender === "F" ? "Femenino" : "Masculino"}</span>
                        )}
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    <div>
                      ...
                      {ficha.grades.length === 0 ? (
                        <div className="rounded-[14px] bg-[#27272a] py-6 text-center">
                        ...
                        <div className="grid grid-cols-4 gap-2">
                          {PERIODS.map(p => {
                            ...
                            return (
                              <div key={p} className="rounded-[14px] bg-[#27272a] p-3 text-center">
                      ...
                      {ficha.attendance.total === 0 ? (
                        ...
                        <>
                          <div className="flex items-center gap-2 mb-3">
                            <div className="flex-1 h-2 rounded-[12px] bg-[#3f3f46] overflow-hidden">
                          ...
                          {ficha.recentAttendance.length > 0 && (
                            <div className="flex gap-1.5 flex-wrap">
                              {ficha.recentAttendance.map(r => (
                                <span key={r.date} className="inline-flex items-center gap-1 text-[9px] font-medium px-1.5 py-0.5 rounded-[12px] bg-[#3f3f46] text-[#a1a1aa]">
                    ...
                    <div>
                      ...
                      {ficha.parents.length === 0 ? (
                        <div className="rounded-[14px] bg-[#27272a] py-5 text-center">
                          <p className="text-xs text-[#a1a1aa]">Sin apoderados vinculados</p>
                        </div>
                      ) : (
                        <div className="space-y-2">
                          {ficha.parents.map(p => (
                            <div key={p.id} className="rounded-[14px] bg-[#27272a] p-3.5 flex items-center gap-3">
                              <div className="h-9 w-9 rounded-[14px] bg-[#3f3f46] flex items-center justify-center shrink-0">
                              ...
                              <div className="flex gap-1 shrink-0">
                                ...
                                {p.phone && (
                                  <a href={`tel:${p.phone}`} title="Llamar"
                                    className="h-8 w-8 rounded-[14px] flex items-center justify-center bg-[#3f3f46] text-[#a1a1aa] hover:text-[#f4f4f5] transition-colors">
                    ...
                    {/* Acciones */}
                    <div className="flex gap-2 pt-2 border-t border-[#3f3f46] dark:border-[#27272a]">
