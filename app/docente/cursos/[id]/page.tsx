  ...
  if (loading) {
    return (
      <div className="space-y-5">
        ...
        <div className="bg-sb-surface rounded-[6px] p-5 animate-pulse space-y-3">
        ...
        {[1, 2, 3].map(i => (
          <div key={i} className="bg-sb-surface rounded-[6px] p-5 animate-pulse">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-[6px] bg-sb-surface-container" />
  ...
  return (
    <div className="space-y-5">
          ...
          return (
            <motion.div key={a.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 + i * 0.04 }}>
              <Link href={a.href} className="block">
                <div className="bg-sb-surface rounded-[6px] p-4 border border-sb-outline-variant/8 hover:border-sb-primary/30 hover:bg-sb-primary/5 transition-all group">
                  <div className="h-9 w-9 rounded-[6px] bg-sb-primary/10 flex items-center justify-center mb-3 group-hover:bg-sb-primary/15 transition-colors">
                    <Icon className="h-4.5 w-4.5 text-sb-primary" />
                  </div>
                  <p className="text-sm font-semibold text-sb-on-surface">{a.label}</p>
                  <p className="text-[11px] text-sb-on-surface-variant/45 mt-0.5">{a.desc}</p>
                </div>
              </Link>
            </motion.div>
          )
      ...
      <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.05 } } }} className="grid grid-cols-2 gap-3">
        <motion.div variants={staggerItem} className="bg-sb-surface rounded-[6px] p-4">
          <div className="h-9 w-9 rounded-[6px] flex items-center justify-center mb-3 bg-blue-500/8">
        ...
        <motion.div variants={staggerItem} className="bg-sb-surface rounded-[6px] p-4">
          <div className="h-9 w-9 rounded-[6px] flex items-center justify-center mb-3 bg-emerald-500/8">
          ...
          {students.length === 0 ? (
            ...
            students.map(s => (
              <motion.button
                ...
                className="w-full text-left bg-sb-surface rounded-[6px] p-4 flex items-center gap-3 hover:bg-sb-surface-container-low/70 hover:ring-1 hover:ring-sb-primary/20 transition-all cursor-pointer group">
                <div className={`h-10 w-10 rounded-[6px] ${getAvatarColor(`${s.first_name} ${s.last_name}`)} flex items-center justify-center shrink-0`}>
                ...
                <span className="inline-flex items-center gap-1.5 text-[10px] font-medium text-sb-primary bg-sb-primary/10 px-2.5 py-1 rounded-[6px] opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                  Ver ficha
                </span>
              </motion.button>
            ))
      ...
      {activeTab === "docentes" && (
        <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.04 } } }} className="space-y-2">
          {teachers.length === 0 ? (
            ...
            teachers.map(t => (
              <motion.div key={t.id} variants={staggerItem} className="bg-sb-surface rounded-[6px] p-4 flex items-center gap-3">
                <div className={`h-10 w-10 rounded-[6px] ${getAvatarColor(t.full_name)} flex items-center justify-center shrink-0`}>
      ...
      <SbCard className="p-4">
        <div className="flex items-start gap-3">
          <div className="h-8 w-8 rounded-[6px] bg-sb-primary/10 flex items-center justify-center shrink-0">
        ...
        {fichaOpen && (
          <motion.div
            ...
            <motion.div
              ...
              className="relative bg-sb-surface rounded-[6px] w-full max-w-[560px] shadow-2xl shadow-black/30 max-h-[90vh] overflow-y-auto">
              {fichaLoading ? (
                ...
                <div>
                  <button onClick={() => setFichaOpen(false)}
                    className="absolute right-3 top-3 h-8 w-8 rounded-[6px] flex items-center justify-center bg-sb-surface-container text-sb-on-surface-variant/60 hover:bg-sb-surface-container-high hover:text-sb-on-surface transition-colors z-10">
                    <X className="h-4 w-4" />
                  </button>

                  {/* Header */}
                  <div className="p-6 pb-5 flex items-center gap-4 bg-sb-surface-container/40 border-b border-sb-outline-variant/10">
                    <div className={`h-16 w-16 rounded-[6px] ${getAvatarColor(`${ficha.student.first_name} ${ficha.student.last_name}`)} flex items-center justify-center shrink-0`}>
                      <span className="text-lg font-bold text-white">{initials(`${ficha.student.first_name} ${ficha.student.last_name}`)}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-lg font-bold text-sb-on-surface truncate">{ficha.student.first_name} {ficha.student.last_name}</p>
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {ficha.student.document_number && (
                          <span className="text-[10px] font-medium px-2 py-0.5 rounded-[6px] bg-sb-surface-container-high text-sb-on-surface-variant/60">DNI: {ficha.student.document_number}</span>
                        )}
                        {ficha.student.gender && (
                          <span className="text-[10px] font-medium px-2 py-0.5 rounded-[6px] bg-sb-surface-container-high text-sb-on-surface-variant/60">{ficha.student.gender === "F" ? "Femenino" : "Masculino"}</span>
                        )}
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <p className="text-[10px] font-semibold text-sb-on-surface-variant/40 uppercase tracking-wider flex items-center gap-1.5">
                          <BookMarked className="h-3.5 w-3.5" /> Notas en el curso
                        </p>
                        {ficha.grades.length > 0 && (
                          <span className="text-sm font-bold text-sb-on-surface">{calcAverage(ficha.grades)}</span>
                        )}
                      </div>
                      {ficha.grades.length === 0 ? (
                        <div className="rounded-[6px] bg-sb-surface-container/50 py-6 text-center">
                          <p className="text-xs text-sb-on-surface-variant/30">Sin calificaciones registradas</p>
                        </div>
                      ) : (
                        <div className="grid grid-cols-4 gap-2">
                          {PERIODS.map(p => {
                            ...
                            return (
                              <div key={p} className="rounded-[6px] bg-sb-surface-container/50 p-3 text-center">
                                <p className="text-[9px] text-sb-on-surface-variant/40 uppercase tracking-wider mb-1.5">B{p.split(" ")[1]}</p>
                                {g ? (
                                  <p className={`text-lg font-bold ${Number(g.score) >= 11 ? "text-emerald-600" : "text-red-500"}`}>{Number(g.score)}</p>
                                ) : (
                                  <p className="text-lg font-bold text-sb-on-surface-variant/20">—</p>
                                )}
                              </div>
                            )
                          })}
                    ...
                    <div>
                      ...
                      {ficha.attendance.total === 0 ? (
                        <div className="rounded-[6px] bg-sb-surface-container/50 py-6 text-center">
                          <p className="text-xs text-sb-on-surface-variant/30">Sin registros de asistencia</p>
                        </div>
                      ) : (
                        <>
                          <div className="flex items-center gap-2 mb-3">
                            <div className="flex-1 h-2 rounded-[6px] bg-sb-surface-container overflow-hidden">
                              <div className={`h-full rounded-[6px] transition-all duration-700 ${ficha.attendance.rate >= 80 ? "bg-emerald-400" : ficha.attendance.rate >= 60 ? "bg-amber-400" : "bg-red-400"}`}
                                style={{ width: `${ficha.attendance.rate}%` }} />
                            </div>
                            <span className="text-sm font-bold text-sb-on-surface">{ficha.attendance.rate}%</span>
                          </div>
                          <div className="grid grid-cols-4 gap-2 mb-3">
                            {[
                              ...
                              <div key={stat.label} className={`rounded-[6px] ${stat.bg} p-2 text-center`}>
                                <p className={`text-base font-bold ${stat.color}`}>{stat.value}</p>
                                <p className="text-[9px] text-sb-on-surface-variant/40">{stat.label}</p>
                              </div>
                            ))}
                          </div>
                          {ficha.recentAttendance.length > 0 && (
                            <div className="flex gap-1.5 flex-wrap">
                              {ficha.recentAttendance.map(r => (
                                <span key={r.date} className="inline-flex items-center gap-1 text-[9px] font-medium px-1.5 py-0.5 rounded-[6px] bg-sb-surface-container text-sb-on-surface-variant/40">
                                  <span className={`h-1.5 w-1.5 rounded-[6px] ${ATT_DOT[r.status] || "bg-sb-on-surface-variant/20"}`} />
                    ...
                    {/* Padres */}
                    <div>
                      <p className="text-[10px] font-semibold text-sb-on-surface-variant/40 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                        <Users className="h-3.5 w-3.5" /> Apoderados ({ficha.parents.length})
                      </p>
                      {ficha.parents.length === 0 ? (
                        <div className="rounded-[6px] bg-sb-surface-container/50 py-5 text-center">
                          <p className="text-xs text-sb-on-surface-variant/30">Sin apoderados vinculados</p>
                        </div>
                      ) : (
                        <div className="space-y-2">
                          {ficha.parents.map(p => (
                            <div key={p.id} className="rounded-[6px] bg-sb-surface-container/50 p-3.5 flex items-center gap-3">
                              <div className="h-9 w-9 rounded-[6px] bg-sb-primary/10 flex items-center justify-center shrink-0">
                                <span className="text-[10px] font-bold text-sb-primary">{initials(`${p.first_name} ${p.last_name}`)}</span>
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium text-sb-on-surface truncate capitalize">{p.first_name} {p.last_name}</p>
                                <p className="text-[10px] text-sb-on-surface-variant/40 capitalize">
                              ...
                              <div className="flex gap-1 shrink-0">
                                {p.phone && (
                                  <a href={`https://wa.me/${String(p.phone).replace(/\D/g, "")}`} target="_blank" rel="noreferrer"
                                    ...
                                    className="h-8 w-8 rounded-[6px] flex items-center justify-center bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 transition-colors">
                                    <MessageSquare className="h-4 w-4" />
                                  </a>
                                )}
                                {p.phone && (
                                  <a href={`tel:${p.phone}`} title="Llamar"
                                    className="h-8 w-8 rounded-[6px] flex items-center justify-center bg-sb-surface-container-high text-sb-on-surface-variant/60 hover:text-sb-on-surface transition-colors">
                                    <Phone className="h-4 w-4" />
                                  </a>
                                )}
                                {p.email && (
                                  <a href={`mailto:${p.email}`} title="Enviar correo"
                                    className="h-8 w-8 rounded-[6px] flex items-center justify-center bg-sb-surface-container-high text-sb-on-surface-variant/60 hover:text-sb-primary transition-colors">
                    ...
                    {/* Acciones */}
                    <div className="flex gap-2 pt-2 border-t border-sb-outline-variant/10">
