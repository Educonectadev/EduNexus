...
export default function CursoDetallePage() {
  ...
  if (loading) {
    return (
      <div className="sb-note mx-auto w-full max-w-[1034px] px-2 pb-4 space-y-5">
        <div className="h-6 w-48 bg-[var(--note-fill-strong)] rounded animate-pulse" />
        <div className="bg-[var(--note-surface)] rounded-[24px] border border-[var(--note-hairline)] p-5 animate-pulse space-y-3">
        ...
        {[1, 2, 3].map(i => (
          <div key={i} className="bg-[var(--note-surface)] rounded-[24px] border border-[var(--note-hairline)] p-5 animate-pulse">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-[12px] bg-[var(--note-fill-strong)]" />
              <div className="space-y-2">
                <div className="h-4 w-32 rounded bg-[var(--note-fill-strong)]" />
                <div className="h-3 w-20 rounded bg-[var(--note-fill-strong)]" />
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    <div className="sb-note">
      ...
      <Link href="/docente/cursos" className="inline-flex items-center gap-1.5 text-sm text-[var(--note-muted)] hover:text-[var(--note-text)] transition-colors">
        <ArrowLeft className="h-4 w-4" />
        Mis Cursos
      </Link>

      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="pt-2">
        <h1 className="text-[26px] sm:text-[30px] leading-tight tracking-[-0.03em] text-[var(--note-text)]">{course.name}</h1>
        <p className="mt-1 text-sm text-[var(--note-muted)]">{course.grade} - Sección {course.section} · Código {course.code}</p>
      </motion.div>
          ...
          return (
            <motion.div key={a.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 + i * 0.04 }}>
              <Link href={a.href} className="block group">
                <div className="bg-[var(--note-surface)] rounded-[24px] border border-[var(--note-hairline)] p-4 group-hover:-translate-y-px group-hover:opacity-90 group-hover:border-[var(--note-hairline-strong)] transition-all">
                  <div className="h-9 w-9 rounded-[12px] bg-[var(--note-fill-strong)] flex items-center justify-center mb-3">
                    <Icon className="h-4 w-4 text-[var(--note-text)]" />
                  </div>
                  <p className="text-sm font-semibold text-[var(--note-text)]">{a.label}</p>
                  <p className="text-[11px] text-[var(--note-muted)] mt-0.5">{a.desc}</p>
                </div>
              </Link>
            </motion.div>
          )
          ...
          {students.length === 0 ? (
            ...
            students.map(s => (
              <motion.button
                ...
                className="w-full text-left bg-[var(--note-surface)] rounded-[24px] border border-[var(--note-hairline)] p-4 flex items-center gap-3 hover:-translate-y-px hover:opacity-90 hover:border-[var(--note-hairline-strong)] transition-all cursor-pointer group">
                <div className={`h-10 w-10 rounded-[12px] ${getAvatarColor(`${s.first_name} ${s.last_name}`)} flex items-center justify-center shrink-0`}>
                  <span className="text-white text-xs font-bold">{initials(`${s.first_name} ${s.last_name}`)}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[var(--note-text)] truncate">{s.first_name} {s.last_name}</p>
                  <p className="text-xs text-[var(--note-muted)]">{s.code}{s.document_number ? ` · DNI ${s.document_number}` : ""}</p>
                </div>
                {s.gender && (
                ...
                <span className="inline-flex items-center gap-1.5 text-[10px] font-medium text-[var(--note-text)] bg-[var(--note-fill-strong)] px-2.5 py-1 rounded-[12px] opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                  Ver ficha
                </span>
              </motion.button>
            ))
      ...
      {activeTab === "docentes" && (
        <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.04 } } }} className="space-y-2">
          {teachers.length === 0 ? (
            ...
            teachers.map(t => (
              <motion.div key={t.id} variants={staggerItem} className="bg-[var(--note-surface)] rounded-[24px] border border-[var(--note-hairline)] p-4 flex items-center gap-3">
                <div className={`h-10 w-10 rounded-[12px] ${getAvatarColor(t.full_name)} flex items-center justify-center shrink-0`}>
                  <span className="text-white text-xs font-bold">{initials(t.full_name)}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium text-[var(--note-text)] truncate">{t.full_name}</p>
                    <BadgeCheck className="h-4 w-4 text-emerald-500 shrink-0" />
                  </div>
                  <p className="text-xs text-[var(--note-muted)] flex items-center gap-1">
      ...
      <div className="bg-[var(--note-surface)] rounded-[24px] border border-[var(--note-hairline)] p-4 flex items-start gap-3">
          <div className="h-8 w-8 rounded-[12px] bg-[var(--note-fill-strong)] flex items-center justify-center shrink-0">
            <BookOpen className="h-4 w-4 text-[var(--note-text)]" />
          </div>
          <div>
            <p className="text-sm font-medium text-[var(--note-text)]">Información del curso</p>
        ...
        {fichaOpen && (
          <motion.div
            ...
            <motion.div
              ...
              className="relative bg-[var(--note-surface)] border border-[var(--note-hairline)] rounded-[24px] w-full max-w-[560px] shadow-2xl shadow-black/30 max-h-[90vh] overflow-y-auto">
              {fichaLoading ? (
                <div className="p-10 flex flex-col items-center gap-3">
                  <div className="h-8 w-8 border-2 border-sb-primary/30 border-t-sb-primary rounded-full animate-spin" />
                ...
                <div>
                  <button onClick={() => setFichaOpen(false)}
                    className="absolute right-3 top-3 h-8 w-8 rounded-[12px] flex items-center justify-center bg-[var(--note-fill-strong)] text-[var(--note-muted)] hover:text-[var(--note-text)] transition-colors z-10">
                    <X className="h-4 w-4" />
                  </button>

                  {/* Header */}
                  <div className="p-6 pb-5 flex items-center gap-4 bg-[var(--note-fill)] border-b border-[var(--note-hairline)]">
                    <div className={`h-16 w-16 rounded-[12px] ${getAvatarColor(`${ficha.student.first_name} ${ficha.student.last_name}`)} flex items-center justify-center shrink-0`}>
                      <span className="text-lg font-bold text-white">{initials(`${ficha.student.first_name} ${ficha.student.last_name}`)}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-lg font-bold text-[var(--note-text)] truncate">{ficha.student.first_name} {ficha.student.last_name}</p>
                      <p className="text-xs text-[var(--note-muted)] mt-0.5">{ficha.course?.name || "Sin curso"} · {ficha.student.grade} &quot;{ficha.student.section}&quot;</p>
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {ficha.student.document_number && (
                          <span className="text-[10px] font-medium px-2 py-0.5 rounded-[12px] bg-[var(--note-fill-strong)] text-[var(--note-muted)]">DNI: {ficha.student.document_number}</span>
                        )}
                        {ficha.student.gender && (
                          <span className="text-[10px] font-medium px-2 py-0.5 rounded-[12px] bg-[var(--note-fill-strong)] text-[var(--note-muted)]">{ficha.student.gender === "F" ? "Femenino" : "Masculino"}</span>
                        )}
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <p className="text-[10px] font-semibold text-[var(--note-muted)] uppercase tracking-wider flex items-center gap-1.5">
                          <BookMarked className="h-3.5 w-3.5" /> Notas en el curso
                        </p>
                        {ficha.grades.length > 0 && (
                          <span className="text-sm font-bold text-[var(--note-text)]">{calcAverage(ficha.grades)}</span>
                        )}
                      </div>
                      {ficha.grades.length === 0 ? (
                        <div className="rounded-[12px] bg-[var(--note-fill)] py-6 text-center">
                          <p className="text-xs text-[var(--note-muted)]">Sin calificaciones registradas</p>
                        </div>
                      ) : (
                        <div className="grid grid-cols-4 gap-2">
                          {PERIODS.map(p => {
                            ...
                            return (
                              <div key={p} className="rounded-[12px] bg-[var(--note-fill)] p-3 text-center">
                                <p className="text-[9px] text-[var(--note-muted)] uppercase tracking-wider mb-1.5">B{p.split(" ")[1]}</p>
                                {g ? (
                                  <p className={`text-lg font-bold ${Number(g.score) >= 11 ? "text-emerald-600" : "text-red-500"}`}>{Number(g.score)}</p>
                                ) : (
                                  <p className="text-lg font-bold text-[var(--note-muted)]">—</p>
                                )}
                              </div>
                            )
                          })}
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-[11px] font-medium text-[var(--note-muted)]">{ficha.attendance.total} registros</span>
                      </div>
                      {ficha.attendance.total === 0 ? (
                        <div className="rounded-[12px] bg-[var(--note-fill)] py-6 text-center">
                          <p className="text-xs text-[var(--note-muted)]">Sin registros de asistencia</p>
                        </div>
                      ) : (
                        <>
                          <div className="flex items-center gap-2 mb-3">
                            <div className="flex-1 h-2 rounded-[12px] bg-[var(--note-fill-strong)] overflow-hidden">
                            ...
                            <span className="text-sm font-bold text-[var(--note-text)]">{ficha.attendance.rate}%</span>
                          </div>
                          <div className="grid grid-cols-4 gap-2 mb-3">
                            {[
                              ...
                              <div key={stat.label} className={`rounded-[12px] ${stat.bg} p-2 text-center`}>
                                <p className={`text-base font-bold ${stat.color}`}>{stat.value}</p>
                                <p className="text-[9px] text-[var(--note-muted)]">{stat.label}</p>
                              </div>
                            ))}
                          </div>
                          {ficha.recentAttendance.length > 0 && (
                            <div className="flex gap-1.5 flex-wrap">
                              {ficha.recentAttendance.map(r => (
                                <span key={r.date} className="inline-flex items-center gap-1 text-[9px] font-medium px-1.5 py-0.5 rounded-[12px] bg-[var(--note-fill-strong)] text-[var(--note-muted)]">
                                  <span className={`h-1.5 w-1.5 rounded-[12px] ${ATT_DOT[r.status] || "bg-[var(--note-muted)]"}`} />
                    ...
                    {/* Padres */}
                    <div>
                      <p className="text-[10px] font-semibold text-[var(--note-muted)] uppercase tracking-wider mb-3 flex items-center gap-1.5">
                        <Users className="h-3.5 w-3.5" /> Apoderados ({ficha.parents.length})
                      </p>
                      {ficha.parents.length === 0 ? (
                        <div className="rounded-[12px] bg-[var(--note-fill)] py-5 text-center">
                          <p className="text-xs text-[var(--note-muted)]">Sin apoderados vinculados</p>
                        </div>
                      ) : (
                        <div className="space-y-2">
                          {ficha.parents.map(p => (
                            <div key={p.id} className="rounded-[12px] bg-[var(--note-fill)] p-3.5 flex items-center gap-3">
                              <div className="h-9 w-9 rounded-[12px] bg-[var(--note-fill-strong)] flex items-center justify-center shrink-0">
                                <span className="text-[10px] font-bold text-[var(--note-text)]">{initials(`${p.first_name} ${p.last_name}`)}</span>
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium text-[var(--note-text)] truncate capitalize">{p.first_name} {p.last_name}</p>
                                <p className="text-[10px] text-[var(--note-muted)] capitalize">
                                  {p.relationship}{p.is_primary ? " · Principal" : ""}{p.occupation ? ` · ${p.occupation}` : ""}
                                </p>
                                {p.phone && <p className="text-[10px] text-[var(--note-muted)] mt-0.5">{p.phone}</p>}
                              </div>
                              <div className="flex gap-1 shrink-0">
                                {p.phone && (
                                  <a href={`https://wa.me/${String(p.phone).replace(/\D/g, "")}`} target="_blank" rel="noreferrer"
                                    title="Escribir por WhatsApp"
                                    className="h-8 w-8 rounded-[12px] flex items-center justify-center bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 transition-colors">
                                    <MessageSquare className="h-4 w-4" />
                                  </a>
                                )}
                                {p.phone && (
                                  <a href={`tel:${p.phone}`} title="Llamar"
                                    className="h-8 w-8 rounded-[12px] flex items-center justify-center bg-[var(--note-fill-strong)] text-[var(--note-muted)] hover:text-[var(--note-text)] transition-colors">
                    ...
                    {/* Acciones */}
                    <div className="flex gap-2 pt-2 border-t border-[var(--note-hairline)]">
