            ...
            {courses.map((c) => (
              <Link key={c.id} href={`/docente/cursos/${c.id}`} className="block">
                <motion.div variants={listItem}
                  className="bg-sb-surface rounded-2xl p-5 hover:bg-sb-surface-container-low/50 transition-colors cursor-pointer group">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`h-11 w-11 rounded-2xl ${getAvatarColor(c.name)} flex items-center justify-center shrink-0`}>
