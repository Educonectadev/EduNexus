    ...
    return (
      <div className="space-y-5">
        <div className="animate-pulse space-y-5">
          <div className="h-8 w-56 rounded-xl bg-sb-surface-container" />
            ...
            <div className="bg-sb-surface rounded-2xl overflow-hidden border border-sb-outline-variant/8">
              ...
              {todaySchedule.length === 0 ? (
                <div className="px-5 pb-5">
                  <div className="rounded-xl bg-sb-surface-container p-4 text-center">
                  ...
                  {todaySchedule.map(h => {
                    ...
                    return (
                      <div key={h.id} className={cn(
                        ...
                        <div className={cn("h-10 w-14 rounded-lg flex flex-col items-center justify-center shrink-0", isNext ? "bg-sb-primary text-sb-on-primary" : "bg-sb-surface-container-high text-sb-on-surface-variant")}>
                ...
                {courses.length === 0 ? (
                  ...
                  <Link key={c.id} href={`/docente/cursos/${c.id}`}>
                    <div className="flex items-center gap-3 px-5 py-3.5 hover:bg-sb-surface-container-low/50 transition-colors group">
                      <div className={cn("h-9 w-9 rounded-xl flex items-center justify-center shrink-0", getAvatarColor(c.name))}>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }}>
            <div className="bg-sb-surface rounded-2xl overflow-hidden border border-sb-outline-variant/8">
              <div className="px-5 pt-5 pb-3 flex items-center justify-between">
                ...
                {attS && (
                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium ${attS.color}`}>
                    <span className={`h-1 w-1 rounded-full ${attS.dot}`} /> {attS.label}
                  </span>
                )}
              </div>
              <div className="px-5 pb-5 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-xl bg-sb-surface-container p-3">
                ...
                {!checkedIn && (
                  <button onClick={() => handleCheck("check-in")}
                    className="w-full h-11 rounded-xl bg-emerald-500 text-white text-sm font-semibold flex items-center justify-center gap-2 hover:bg-emerald-400 transition-all">
                    <LogIn className="h-4 w-4" /> Marcar Entrada
                  </button>
                )}
                {checkedIn && !checkedOut && (
                  <button onClick={() => handleCheck("check-out")}
                    className="w-full h-11 rounded-xl bg-amber-500 text-white text-sm font-semibold flex items-center justify-center gap-2 hover:bg-amber-400 transition-all">
                    <LogOut className="h-4 w-4" /> Marcar Salida
                  </button>
                )}
                {checkedIn && checkedOut && (
                  <Link href="/docente/asistencia">
                    <div className="w-full h-11 rounded-xl bg-emerald-500/10 text-emerald-600 text-sm font-semibold flex items-center justify-center gap-2 hover:bg-emerald-500/15 transition-all">
                  ...
                  return (
                    <Link key={a.label} href={a.href}>
                      <div className="flex items-center gap-3 px-5 py-3 hover:bg-sb-surface-container-low/50 transition-colors group">
                        <div className="h-8 w-8 rounded-lg bg-sb-surface-container flex items-center justify-center shrink-0 group-hover:bg-sb-primary/10 transition-colors">
