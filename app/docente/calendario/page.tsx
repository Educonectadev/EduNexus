...
export default function CalendarioPage() {
  ...
  return (
    <div className="w-full space-y-6 py-2">
      ...
      {/* Calendar grid */}
      <div className="bg-[var(--sb-surface-container)] rounded-2xl p-4">
        <div className="flex items-center justify-between mb-4">
          <button onClick={() => { if (currentMonth === 0) { setCurrentMonth(11); setCurrentYear(y => y - 1) } else setCurrentMonth(m => m - 1) }} className="p-2 rounded-xl hover:bg-[var(--sb-surface-container-high)] transition-colors">
          ...
          <button onClick={() => { if (currentMonth === 11) { setCurrentMonth(0); setCurrentYear(y => y + 1) } else setCurrentMonth(m => m + 1) }} className="p-2 rounded-xl hover:bg-[var(--sb-surface-container-high)] transition-colors">
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <div key={day} className={cn("relative flex flex-col items-center py-2 rounded-xl text-sm transition-colors", isToday ? "bg-[var(--sb-primary)] text-[var(--sb-on-primary)]" : "text-[var(--sb-on-surface)]/70 hover:bg-[var(--sb-surface-container-high)]")}>
                {day}
                {dayEvents.length > 0 && (
                  <div className="flex gap-0.5 mt-0.5">
                    {dayEvents.slice(0, 3).map((_, j) => <div key={j} className="h-1 w-1 rounded-full bg-[var(--sb-primary)]" />)}
                  </div>
                )}
              </div>
            )
      ...
      <div className="space-y-3">
        ...
        {upcomingEvents.map((e, i) => (
          <motion.div key={e.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-[var(--sb-surface-container)] rounded-2xl p-4 flex items-center gap-4">
            <div className={cn("h-10 w-10 rounded-2xl flex items-center justify-center shrink-0", typeColors[e.type])}>
            ...
            <span className={cn("text-[10px] font-medium px-2.5 py-1 rounded-full shrink-0", typeColors[e.type])}>{typeLabels[e.type]}</span>
          </motion.div>
        ))}
      </div>
    </div>
