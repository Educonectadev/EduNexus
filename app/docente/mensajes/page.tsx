      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="space-y-3">
        <div className="relative">
          ...
          <input placeholder="Buscar por nombre, asunto o contenido..." value={search} onChange={e => setSearch(e.target.value)}
            className="sb-input rounded-2xl text-sm h-12 w-full pl-11 pr-10 bg-sb-surface-container/50 border-sb-outline-variant/10 focus:bg-sb-surface focus:border-sb-primary/30 transition-all" />
          {search && (
            <button onClick={() => setSearch("")}
              className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 rounded-full hover:bg-sb-surface-container transition-colors">
      ...
      <div className="bg-sb-surface rounded-2xl overflow-hidden border border-sb-outline-variant/8">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    {!m.read && <span className="h-2 w-2 rounded-full bg-sb-primary animate-pulse" />}
        ...
        {filtered.length === 0 && (
          <div className="py-14 text-center">
            <div className="h-14 w-14 rounded-2xl bg-sb-surface-container flex items-center justify-center mx-auto mb-4">
      ...
      <AnimatePresence>
        {selected && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            className="bg-sb-surface rounded-2xl overflow-hidden border border-sb-outline-variant/8">
            <div className="flex items-center justify-between px-5 py-4 border-b border-sb-outline-variant/8 bg-sb-surface-container/30">
              <div className="flex items-center gap-3">
                <div className={`h-10 w-10 rounded-xl ${getAvatarColor(selected.from)} flex items-center justify-center shrink-0`}>
              ...
              <div className="flex gap-2">
                <div className="flex-1 relative">
                  <input placeholder="Escribir respuesta..." value={reply} onChange={e => setReply(e.target.value)}
                    className="sb-input rounded-2xl text-sm h-11 w-full pr-12 bg-sb-surface-container/50" />
                  <button className={`absolute right-2 top-1/2 -translate-y-1/2 p-2.5 rounded-xl transition-all ${
