...
function generateEmail(name: string): string {
  const parts = name.toLowerCase()
    ...
    .trim()
    .split(/\s+/)
  const first = parts[0] || 'sec'
  const last = parts[parts.length - 1] || ''
  return `sec.${first}.${last}@iep.edu.pe`
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const areaIdx = headers.findIndex(h => h.includes('área') || h.includes('area') || h.includes('departamento') || h.includes('department'))
    ...
    const [existingUsers] = await pool.query(
      `SELECT dni FROM users WHERE institution_id = $1 AND dni IS NOT NULL AND dni != ''`,
      [institutionId]
    ) as any[]
    const existingDnis = new Set(existingUsers.map((u: any) => u.dni))

    const [existingEmails] = await pool.query(
      `SELECT email FROM users WHERE institution_id = $1 AND email IS NOT NULL AND email != ''`,
      [institutionId]
    ) as any[]
    ...
    const rows: Array<{
      ...
      area: string; contract: string; status: string;
      lineNum: number
    }> = []
    ...
    for (let batch = 0; batch < rows.length; batch += BATCH_SIZE) {
      ...
      for (const row of chunk) {
        ...
        values.push([
          id, email, row.name, hashedPassword, 'secretario',
          institutionId, row.dni, row.phone, row.area,
          '', row.contract || '', row.status || 'active'
        ])
