        ...
        {selectedInstitution && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50" onClick={() => setSelectedInstitution(null)}>
            <motion.div
              ...
              className="fixed inset-x-3 bottom-3 top-auto max-h-[85vh] overflow-y-auto rounded-3xl sm:inset-x-0 sm:left-1/2 sm:-translate-x-1/2 sm:top-1/2 sm:-translate-y-1/2 sm:bottom-auto sm:max-w-2xl sm:rounded-2xl bg-sb-surface border border-sb-outline-variant/10"
        ...
        {resetModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50" onClick={() => { setResetModal(null); setNewPassword(""); setShowPassword(false) }}>
            <motion.div
              ...
              exit={{ opacity: 0, y: 40 }}
              transition={{ type: "spring", damping: 28, stiffness: 320 }}
              className="fixed inset-x-3 bottom-3 top-auto max-h-[92vh] overflow-y-auto rounded-3xl sm:inset-x-0 sm:left-1/2 sm:-translate-x-1/2 sm:top-1/2 sm:-translate-y-1/2 sm:bottom-auto sm:max-w-lg sm:rounded-2xl bg-sb-surface border border-sb-outline-variant/10"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="sticky top-0 bg-sb-surface/95 backdrop-blur flex items-center justify-between px-6 py-4 border-b border-sb-outline-variant/10">
